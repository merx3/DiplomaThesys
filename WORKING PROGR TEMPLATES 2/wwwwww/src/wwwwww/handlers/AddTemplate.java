...
public class AddTemplate extends AbstractTemplatesPage
{
     ...
     * @param javaEditor
     */
    public AddTemplate(ITextEditor javaEditor, ISourceViewer viewer)
    {
    ...
    protected boolean isValidTemplate(IDocument document, Template template, int offset, int length)
    {
        ...
        {
        	System.out.println(contextIds[i] + "-" + template.getContextTypeId() );
    ...
    private DocumentTemplateContext getContext(IDocument document, Template template, final int offset, int length)
    {
        ...
        if (template.getContextTypeId().equals(JavaDocContextType.ID))
        {
            context = new JavaDocContext(getContextTypeRegistry().getContextType(template.getContextTypeId()), document,
                    new Position(offset, length), (ICompilationUnit) EditorUtility
        ...
        else
        {
            context = new JavaContext(getContextTypeRegistry().getContextType(template.getContextTypeId()), document, new Position(offset, length),
                    (ICompilationUnit) EditorUtility.getEditorInputJavaElement(
                            fJavaEditor, true));
        }
        return context;
    }
